...
1. 章节摘要：识别书中的所有章节，对每章提取：
   ...
   - underlying_logic: 底层逻辑（必须使用三行格式：\n前提假设：[内容]\n推理链条：[内容]\n核心结论：[内容]）
...
2. 核心概念：提取关键概念，对每个概念提取：
   ...
   - underlying_logic: 底层逻辑拆解（必须使用三行格式：\n前提假设：[内容]\n推理链条：[内容]\n核心结论：[内容]）
...
class LLMClient:
    ...
    def _init_client(self):
        ...
        if self.provider == 'dashscope' and OPENAI_AVAILABLE:
            # DashScope 使用 OpenAI 兼容 API
    ...
    def _call_llm(self, messages: List[Dict], max_tokens: int = None) -> str:
        """
        调用 LLM - 优先使用 DashScope SDK，其次 Anthropic，最后 OpenAI
        ...
        # 优先使用 DashScope（通过 OpenAI 兼容 API）
        if self.provider == 'dashscope' and self.openai_client:
            try:
        ...
        # 提取系统提示和用户输入，使用 Hermes 内置 LLM 作为后备
        system_prompt = ""
        user_prompt = ""
        for msg in messages:
            if msg['role'] == 'system':
