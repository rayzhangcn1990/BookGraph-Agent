"""提示词定义模块"""
SYSTEM_PROMPT = """你是一位专业的学术书籍分析助手，专精于知识图谱构建。你的任务是分析书籍内容并输出结构化的 JSON 数据。

约束条件：
- 只输出有效的 JSON，不要有任何其他文本或解释
- 使用指定的英文字段名
- 禁止使用占位符文本如 "TBD"、"N/A"、"待分析"
- 所有内容必须有实质内容，基于提供的文本
- 数组不能为 null；如果没有数据，使用空数组 []
- 所有内容必须使用中文输出（除非是专有名词或英文原文引用）"""


CHUNK_ANALYSIS_PROMPT = """【核心约束 - 必须遵守】
🔴 **语言约束**：所有输出内容必须使用中文（除非是专有名词或英文原文引用）
🔴 **书名约束**：生成的书籍名称必须与源书籍名称完全一致，不得修改、简化或调整
🔴 **格式约束**：只输出有效的 JSON，不要有任何其他文本
🔴 **字段约束**：使用指定的英文字段名，但字段值必须是中文
...
【JSON 格式要求】（使用以下英文字段名，但内容必须为中文）：
{{
  "chapter_summaries": ["每个章节的简要摘要（中文）"],
  "core_concepts": ["从内容中提取的关键概念名称（中文）"],
  "key_insights": ["重要的洞见或论点（中文）"],
  "key_quotes": ["书中出现的值得注意的引用（中文）"]
}}
...
🔴 **语言约束**：所有输出内容必须使用中文（除非是专有名词或英文原文引用）
🔴 **书名约束**：生成的书籍名称必须与源书籍名称完全一致，不得修改、简化或调整
🔴 **严禁占位符**：禁止使用 "待分析"、"待补充"、"TBD"、"TODO"、"N/A"、"书中未涉及" 等占位符
🔴 **底层逻辑格式**：必须使用单行箭头格式：前提假设：[内容]→推理链条：[内容]→核心结论：[内容]
🔴 **内容真实性**：所有内容必须有实质性信息，基于书籍实际内容
...
BATCH_CHUNK_ANALYSIS_PROMPT = """【核心约束 - 必须遵守】
🔴 **语言约束**：所有输出内容必须使用中文（除非是专有名词或英文原文引用）
🔴 **书名约束**：生成的书籍名称必须与源书籍名称完全一致，不得修改、简化或调整
🔴 **严禁占位符**：禁止使用 "待分析"、"待补充"、"TBD"、"TODO"、"N/A" 等占位符
🔴 **底层逻辑格式**：必须使用单行箭头格式：前提假设：[内容]→推理链条：[内容]→核心结论：[内容]

请分析以下多个书籍内容块，提取结构化信息。
...
1. 🔴 **所有内容必须使用中文**（除非是专有名词或英文原文引用）
2. **禁止使用注释**：JSON 不支持 `//` 注释，你的输出必须是无注释的有效 JSON
3. **禁止省略内容**：不允许写 "此处省略"、"实际输出时请完整复制" 等说明
4. **禁止修改结构**：保持原有的 chapter_number、title、core_argument 等字段结构
5. **必须完整输出**：所有 {chapters_count} 个章节都必须出现在最终输出的 chapters 数组中
6. **禁止章节合并**：每个 chapter_number 必须是单个数字（如"1"、"22"），禁止用"11-22"合并
