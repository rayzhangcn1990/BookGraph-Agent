...
async def synthesize_multi_round(
    ...
    # 🔑 修复：使用正确的 field_type（Round 4 只在章节数>20时执行）
    result_4 = await _call_llm_round(llm_client, prompt_4, "round_4", field_type="synthesis_round_4") if len(chapters_json) > 20 else {"chapters": [], "core_concepts": []}
    ...
    if result_4 and "chapters" in result_4:
